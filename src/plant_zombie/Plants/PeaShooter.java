...
public class PeaShooter extends Plant{
    ...
    public PeaShooter(int row, int col, int power, ArrayList<ArrayList<Plant>> matPlant, ArrayList<ArrayList<Zombie>> matZombie) {
        super(row, col, power, matPlant, matZombie);
        setImg(new ImageIcon("src\\plant_zombie\\Image\\peashooter.gif").getImage());
        pea=new Pea(getX()+80, getY()+10);
    }
    ...
    public void Draw(Graphics2D g2d, ImageObserver observer) {
        if(getHealth()>0){
          if(isIsSet()){
              g2d.drawImage(getImgSet(), getX(), getY()+60, 100, 100,  observer);
            }
